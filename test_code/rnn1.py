...
cells = tf.nn.rnn_cell.MultiRNNCell(
    [cell(num_units) for _ in range(num_layer)])
...
with tf.Session() as sess:
    ...
    for step in range(total_steps + 1):
        ...
        sess.run(train, feed_dict={
                 x: batch_x, y_label: batch_y, keep_prob: 0.5, batch_size: batch_x.shape[0]})
        # Train Accuracy
